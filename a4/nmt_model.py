...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, last_output = self.encoder(pack_padded_sequence(X, source_lengths))
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.transpose(enc_hiddens, 0, 1)

        init_decoder_hidden = self.h_projection(torch.cat([t for t in last_output[0]], dim=1))
        init_decoder_cell = self.c_projection(torch.cat([t for t in last_output[1]], dim=1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(torch.unsqueeze(dec_hidden, dim=1),torch.transpose(enc_hiddens_proj, dim0=1,dim1=2))
